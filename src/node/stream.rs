...
fn ee_init(instance: &JsObject, ctx: &mut Context) -> JsResult<()> {
    if instance.has_own_property(js_string!("_events"), ctx).unwrap_or(false) {
        return Ok(());
    }
...
fn get_or_create_arr(events: &JsObject, name: &str, ctx: &mut Context) -> JsResult<JsArray> {
    let val = events.get(js_string!(name), ctx).unwrap_or(JsValue::undefined());
...
fn add_listener_impl(instance: &JsObject, name: &str, listener: &JsValue, ctx: &mut Context) -> JsResult<()> {
...
fn emit_internal(instance: &JsObject, name: &str, emit_args: &[JsValue], ctx: &mut Context) -> JsResult<JsValue> {
    let events = match get_events(instance, ctx) {
        Ok(e) => e,
        Err(_) => return Ok(JsValue::from(false)),
    };
    let val = events.get(js_string!(name), ctx).unwrap_or(JsValue::undefined());
    let arr = match val.as_object().and_then(|o| JsArray::from_object(o.clone()).ok()) {
        Some(a) => a,
...
fn remove_listener_impl(instance: &JsObject, name: &str, listener: &JsValue, ctx: &mut Context) -> JsResult<()> {
    if !instance.has_own_property(js_string!("_events"), ctx).unwrap_or(false) {
        return Ok(());
    }
    let events = match get_events(instance, ctx) {
    ...
    let keep: Vec<JsValue> = (0..arr.length(ctx).unwrap_or(0))
        ...
        .filter(|item| {
            match (item.as_object(), listener.as_object()) {
    ...
    if keep.is_empty() {
        ...
        let _ = events.set(js_string!(name), JsValue::from(JsArray::from_iter(keep, ctx)), false, ctx);
    }
    Ok(())
}
...
fn set_methods(inst: &JsObject, methods: &[(&str, NativeFunction, usize)], ctx: &mut Context) {
    for &(n, ref f, l) in methods {
        let fv = FunctionObjectBuilder::new(ctx.realm(), f.clone()).name(n).length(l).build();
...
fn make_ctor(name: &str, ctor_fn: NativeFunction, len: usize, ctx: &mut Context) -> JsValue {
    let ctor = FunctionObjectBuilder::new(ctx.realm(), ctor_fn).name(name).length(len).constructor(true).build();
...
fn r_state(inst: &JsObject, ctx: &mut Context) -> JsResult<JsObject> {
    if let Ok(v) = inst.get(js_string!("_r"), ctx) {
        if let Some(o) = v.as_object() { return Ok(o); }
    }
    let s = JsObject::with_object_proto(ctx.intrinsics());
    let _ = s.set(js_string!("b"), JsValue::from(JsArray::new(ctx)), false, ctx);
...
fn resume(inst: &JsObject, this: &JsValue, ctx: &mut Context) -> JsResult<()> {
    ...
    let ed = s.get(js_string!("e"), ctx).ok().and_then(|v| v.as_boolean()).unwrap_or(false);
    if f.as_boolean() != Some(true) && !ed {
        let _ = s.set(js_string!("f"), JsValue::from(true), false, ctx);
        if let Some(fn_val) = inst.get(js_string!("_rfn"), ctx).ok()
...
fn readable_push(this: &JsValue, args: &[JsValue], ctx: &mut Context) -> JsResult<JsValue> {
                ...
                boa_engine::job::GenericJob::new(
                    move |jctx| {
                        if let Ok(st) = r_state(&ic, jctx) {
                            if !st.get(js_string!("ee"), jctx).ok().and_then(|v| v.as_boolean()).unwrap_or(false) {
    ...
    if s.get(js_string!("f"), ctx).unwrap_or(JsValue::undefined()).as_boolean() == Some(true) {
...
fn readable_read(this: &JsValue, _args: &[JsValue], ctx: &mut Context) -> JsResult<JsValue> {
    ...
    if ed && buf.length(ctx).unwrap_or(0) == 0 {
        if !s.get(js_string!("ee"), ctx).ok().and_then(|v| v.as_boolean()).unwrap_or(false) {
...
fn stream_pipe(this: &JsValue, args: &[JsValue], ctx: &mut Context) -> JsResult<JsValue> {
    ...
    let on_data = unsafe {
        NativeFunction::from_closure(move |_: &JsValue, da: &[JsValue], ctx: &mut Context| -> JsResult<JsValue> {
            let data = da.first().cloned().unwrap_or(JsValue::undefined());
            if let Some(wf) = dv_for_closure.as_object()
    ...
    let _ = add_listener_impl(&src_inst, "data", &JsValue::from(
        FunctionObjectBuilder::new(ctx.realm(), on_data).name("").length(1).build()
    ), ctx);
    ...
    let on_drain = unsafe {
        NativeFunction::from_closure(move |_: &JsValue, _: &[JsValue], ctx: &mut Context| -> JsResult<JsValue> {
            if let Some(rfn) = ic.get(js_string!("resume"), ctx).ok()
                .and_then(|f| f.as_object().filter(|o| o.is_callable()))
            {
    ...
    let _ = add_listener_impl(&dest, "drain", &JsValue::from(
        FunctionObjectBuilder::new(ctx.realm(), on_drain).name("").length(0).build()
    ), ctx);

    let dc = dest.clone();
    let on_end = unsafe {
        NativeFunction::from_closure(move |_: &JsValue, _: &[JsValue], ctx: &mut Context| -> JsResult<JsValue> {
            if let Some(ef) = dc.get(js_string!("end"), ctx).ok()
                .and_then(|f| f.as_object().filter(|o| o.is_callable()))
            {
    ...
    let _ = add_listener_impl(&src_inst, "end", &JsValue::from(
        FunctionObjectBuilder::new(ctx.realm(), on_end).name("").length(0).build()
    ), ctx);
...
fn writable_write(this: &JsValue, args: &[JsValue], ctx: &mut Context) -> JsResult<JsValue> {
    ...
    let cb = args.get(2).or_else(|| {
        if encoding.as_object().is_some_and(|o| o.is_callable()) { Some(&encoding) } else { None }
    }).cloned().unwrap_or(JsValue::undefined());

    let s = w_state(&inst, ctx)?;
    if s.get(js_string!("e"), ctx).ok().and_then(|v| v.as_boolean()).unwrap_or(false) {
        return Ok(JsValue::from(false));
    }

    let writing = s.get(js_string!("w"), ctx).ok().and_then(|v| v.as_boolean()).unwrap_or(false);
    let buf = get_or_create_arr(&s, "b", ctx)?;
    if writing {
        ...
        let on_done_jv = JsValue::from(FunctionObjectBuilder::new(ctx.realm(), on_done).name("").length(1).build());
        if let Some(wf) = inst.get(js_string!("_wf"), ctx).ok()
            .and_then(|f| f.as_object().filter(|o| o.is_callable()))
        {
...
fn writable_end(this: &JsValue, args: &[JsValue], ctx: &mut Context) -> JsResult<JsValue> {
    ...
    let cb = if is_fn { chunk.clone() } else { args.get(1).cloned().unwrap_or(JsValue::undefined()) };
    ...
    if chunk.is_undefined() || chunk.is_null() {
        if let Some(ff) = inst.get(js_string!("_ff"), ctx).ok()
            .and_then(|f| f.as_object().filter(|o| o.is_callable()))
        {
            let ic = inst.clone();
            let cbc = cb.clone();
            ...
            let ofv = JsValue::from(FunctionObjectBuilder::new(ctx.realm(), on_final).name("").length(0).build());
            let _ = ff.call(&JsValue::undefined(), &[ofv], ctx);
        }
    } else {
        if let Some(wf) = inst.get(js_string!("write"), ctx).ok()
            .and_then(|f| f.as_object().filter(|o| o.is_callable()))
        {
...
fn pipeline_impl(_this: &JsValue, args: &[JsValue], ctx: &mut Context) -> JsResult<JsValue> {
    ...
    if let Some(pf) = args[0].as_object()
    ...
    if let Some(cb_val) = args.get(2) {
        if let Some(cb_obj) = cb_val.as_object().filter(|o| o.is_callable()) {
            if let Some(do_) = args[1].as_object() {
                ...
                let _ = add_listener_impl(&do_, "finish", &JsValue::from(
                    FunctionObjectBuilder::new(ctx.realm(), of).name("").length(0).build()
                ), ctx);
            }
        }
    }
...
fn finished_impl(_this: &JsValue, args: &[JsValue], ctx: &mut Context) -> JsResult<JsValue> {
    ...
    let d1 = done.clone(); let s1 = stream.clone(); let c1 = cb_val.clone();
    let on_end = unsafe {
        NativeFunction::from_closure(move |_: &JsValue, _: &[JsValue], ctx: &mut Context| -> JsResult<JsValue> {
            if !d1.replace(true) {
    ...
    let d2 = done.clone(); let s2 = stream.clone(); let c2 = cb_val.clone();
    ...
    let d3 = done.clone(); let s3 = stream.clone(); let c3 = cb_val.clone();
    let on_error = unsafe {
        NativeFunction::from_closure(move |_: &JsValue, ea: &[JsValue], ctx: &mut Context| -> JsResult<JsValue> {
            if !d3.replace(true) {
                ...
                if let Some(f) = c3.as_object().filter(|o| o.is_callable()) {
                    let _ = f.call(&JsValue::undefined(), &[ea.first().cloned().unwrap_or(JsValue::undefined())], ctx);
                }
            }
            Ok(JsValue::undefined())
        })
    ...
    let _ = add_listener_impl(&stream, "end", &JsValue::from(FunctionObjectBuilder::new(ctx.realm(), on_end).name("").length(0).build()), ctx);
    let _ = add_listener_impl(&stream, "finish", &JsValue::from(FunctionObjectBuilder::new(ctx.realm(), on_finish).name("").length(0).build()), ctx);
    let _ = add_listener_impl(&stream, "error", &JsValue::from(FunctionObjectBuilder::new(ctx.realm(), on_error).name("").length(1).build()), ctx);
    ...
    Ok(JsValue::from(FunctionObjectBuilder::new(ctx.realm(), cleanup).name("").length(0).build()))
}

pub fn create_node_stream_module(context: &mut Context) -> Result<Module, String> {
    let export_names: &[JsString] = &[
        js_string!("Stream"), js_string!("Readable"), js_string!("Writable"),
        js_string!("Duplex"), js_string!("Transform"), js_string!("PassThrough"),
        js_string!("pipeline"), js_string!("finished"), js_string!("default"),
    ];
        ...
        unsafe {
            SyntheticModuleInitializer::from_closure(
                move |m: &boa_engine::module::SyntheticModule, ctx: &mut Context| {
                    let stream = make_ctor("Stream", make_native(|_: &JsValue, _: &[JsValue], ctx: &mut Context| -> JsResult<JsValue> {
                        let inst = JsObject::with_object_proto(ctx.intrinsics());
                        let _ = ee_init(&inst, ctx);
                        let _ = inst.set(js_string!("_d"), JsValue::from(false), false, ctx);
                        set_methods(&inst, &[
                    ...
                    let readable = make_ctor("Readable", make_native(|_: &JsValue, args: &[JsValue], ctx: &mut Context| -> JsResult<JsValue> {
                        let opts = args.first().cloned().unwrap_or(JsValue::undefined());
                        let inst = JsObject::with_object_proto(ctx.intrinsics());
                        let _ = ee_init(&inst, ctx);
                        let _ = r_state(&inst, ctx);
                        let ur = opts.as_object()
                            .and_then(|o| o.get(js_string!("read"), ctx).ok())
                            .filter(|v| !v.is_undefined());
                        let _ = inst.set(js_string!("_rfn"), ur.unwrap_or_else(|| build_fn(make_native(readable_do_read), "_rfn", 1, ctx)), false, ctx);
                        let _ = inst.set(js_string!("_d"), JsValue::from(false), false, ctx);
                        set_methods(&inst, &[
                    ...
                    let writable = make_ctor("Writable", make_native(|_: &JsValue, args: &[JsValue], ctx: &mut Context| -> JsResult<JsValue> {
                        let opts = args.first().cloned().unwrap_or(JsValue::undefined());
                        let inst = JsObject::with_object_proto(ctx.intrinsics());
                        let _ = ee_init(&inst, ctx);
                        ...
                        if let Some(o) = opts.as_object() {
                            if let Ok(fn_val) = o.get(js_string!("write"), ctx) {
                                if !fn_val.is_undefined() { let _ = inst.set(js_string!("_wf"), fn_val, false, ctx); }
                            }
                        }
                    ...
                    let duplex = make_ctor("Duplex", make_native(|_: &JsValue, args: &[JsValue], ctx: &mut Context| -> JsResult<JsValue> {
                        let opts = args.first().cloned().unwrap_or(JsValue::undefined());
                        let inst = JsObject::with_object_proto(ctx.intrinsics());
                        let _ = ee_init(&inst, ctx);
                        let _ = r_state(&inst, ctx);
                        ...
                        if let Some(o) = opts.as_object() {
                            if let Ok(fn_val) = o.get(js_string!("read"), ctx) {
                                if !fn_val.is_undefined() { let _ = inst.set(js_string!("_rfn"), fn_val, false, ctx); }
                            }
                            if let Ok(fn_val) = o.get(js_string!("write"), ctx) {
                                if !fn_val.is_undefined() { let _ = inst.set(js_string!("_wf"), fn_val, false, ctx); }
                            }
                    ...
                    let transform = make_ctor("Transform", make_native(|_: &JsValue, args: &[JsValue], ctx: &mut Context| -> JsResult<JsValue> {
                        let opts = args.first().cloned().unwrap_or(JsValue::undefined());
                        let inst = JsObject::with_object_proto(ctx.intrinsics());
                        let _ = ee_init(&inst, ctx);
                        let _ = r_state(&inst, ctx);
                        ...
                        let _wf = make_native(|_t: &JsValue, _a: &[JsValue], _c: &mut Context| -> JsResult<JsValue> {
                            let chunk = _a.first().cloned().unwrap_or(JsValue::undefined());
                            if !chunk.is_undefined() && !chunk.is_null() {
                                let _ = readable_push(_t, &[chunk], _c);
                            }
                            if let Some(cb) = _a.get(2).and_then(|v| v.as_object()).filter(|o| o.is_callable()) {
                                let _ = cb.call(&JsValue::undefined(), &[], _c);
                            }
                            Ok(JsValue::undefined())
                        });
                        let _ = inst.set(js_string!("_wf"), JsValue::from(FunctionObjectBuilder::new(ctx.realm(), _wf).name("_wf").length(3).build()), false, ctx);
                        set_methods(&inst, &[
                            ("push", make_native(readable_push), 2),
                            ("read", make_native(readable_read), 1),
                            ("resume", make_native(readable_resume), 0),
                    ...
                    let passthrough = make_ctor("PassThrough", make_native(|_: &JsValue, args: &[JsValue], ctx: &mut Context| -> JsResult<JsValue> {
                        let opts = args.first().cloned().unwrap_or(JsValue::undefined());
                        let inst = JsObject::with_object_proto(ctx.intrinsics());
                        let _ = ee_init(&inst, ctx);
                        let _ = r_state(&inst, ctx);
                    ...
                    for &(n, ref v) in &[
                        ...
                        ("pipeline", &pipeline), ("finished", &finished_fn),
                    ] {
