...
fn get_max_listeners(instance: &JsObject, ctx: &mut Context) -> f64 {
    if !instance.has_own_property(js_string!("_maxListeners"), ctx).unwrap_or(false) {
        let _ = ensure_instance_init(instance, ctx);
    }
    instance.get(js_string!("_maxListeners"), ctx)
...
fn add_listener_impl(instance: &JsObject, name: &str, listener: &JsValue, prepend: bool, ctx: &mut Context) -> JsResult<()> {
    let events = get_events(instance, ctx)?;

    if name != "newListener" && has_event(&events, "newListener", ctx) {
        let _ = emit_internal(instance, "newListener", &[JsValue::from(JsString::from(name)), listener.clone()], ctx);
    }
    ...
    if (len as f64) > max && len < 1000 {
        ...
        let _ = emit_internal(instance, "warning", &[JsValue::from(JsString::from(msg))], ctx);
    }
...
fn once_method(this: &JsValue, args: &[JsValue], ctx: &mut Context) -> JsResult<JsValue> {
    ...
    let listener_obj = listener_val.as_object().ok_or_else(|| js_err("listener must be a function"))?;
    ...
    let wrapper = unsafe {
        NativeFunction::from_closure(move |inner_this: &JsValue, inner_args: &[JsValue], inner_ctx: &mut Context| -> JsResult<JsValue> {
...
fn prepend_listener_method(this: &JsValue, args: &[JsValue], ctx: &mut Context) -> JsResult<JsValue> {
    let instance = get_obj(this)?;
    let name = args.first().and_then(|v| v.to_string(ctx).ok()).map(|s| s.to_std_string_escaped()).unwrap_or_default();
    let listener = args.get(1).cloned().unwrap_or(JsValue::undefined());
...
fn prepend_once_listener_method(this: &JsValue, args: &[JsValue], ctx: &mut Context) -> JsResult<JsValue> {
    let instance = get_obj(this)?;
    let name = args.first().and_then(|v| v.to_string(ctx).ok()).map(|s| s.to_std_string_escaped()).unwrap_or_default();
    let listener_val = args.get(1).cloned().unwrap_or(JsValue::undefined());
    let listener_obj = listener_val.as_object().ok_or_else(|| js_err("listener must be a function"))?;
...
fn remove_listener_method(this: &JsValue, args: &[JsValue], ctx: &mut Context) -> JsResult<JsValue> {
    let instance = get_obj(this)?;
    let name = args.first().and_then(|v| v.to_string(ctx).ok()).map(|s| s.to_std_string_escaped()).unwrap_or_default();
    let listener = args.get(1).cloned().unwrap_or(JsValue::undefined());
...
fn remove_all_listeners_method(this: &JsValue, args: &[JsValue], ctx: &mut Context) -> JsResult<JsValue> {
    let instance = get_obj(this)?;
    if let Some(ev) = args.first() {
        let name = ev.to_string(ctx).map(|s| s.to_std_string_escaped()).unwrap_or_default();
        if let Ok(events) = get_events(&instance, ctx) {
...
fn listeners_method(this: &JsValue, args: &[JsValue], ctx: &mut Context) -> JsResult<JsValue> {
    ...
    let name = args.first().and_then(|v| v.to_string(ctx).ok()).map(|s| s.to_std_string_escaped()).unwrap_or_default();
    let events = get_events(&instance, ctx)?;
    let val = events.get(js_string!(name.as_str()), ctx).unwrap_or(JsValue::undefined());
    if val.is_undefined() {
...
fn get_max_listeners_method(this: &JsValue, _args: &[JsValue], ctx: &mut Context) -> JsResult<JsValue> {
    let instance = get_obj(this)?;
    let n = get_max_listeners(&instance, ctx);
    Ok(JsValue::from(n))
}

fn set_max_listeners_method(this: &JsValue, args: &[JsValue], ctx: &mut Context) -> JsResult<JsValue> {
    let instance = get_obj(this)?;
    let n = args.first().and_then(|v| v.as_number()).ok_or_else(|| js_err("n must be a non-negative number"))?;
    if n < 0.0 {
...
fn static_listener_count_method(_this: &JsValue, args: &[JsValue], ctx: &mut Context) -> JsResult<JsValue> {
    let emitter = args.first().ok_or_else(|| js_err("listenerCount requires emitter"))?;
...
fn static_once_method(_this: &JsValue, args: &[JsValue], ctx: &mut Context) -> JsResult<JsValue> {
    let emitter_val = args.first().ok_or_else(|| js_err("EventEmitter.once requires emitter"))?;
    let emitter = get_obj(emitter_val)?;
    let event_name = args.get(1).cloned().unwrap_or(JsValue::undefined());
    let name = event_name.to_string(ctx).map(|s| s.to_std_string_escaped()).unwrap_or_default();

    let (promise, resolvers) = JsPromise::new_pending(ctx);
    let resolve = resolvers.resolve;

    let handler = unsafe {
        NativeFunction::from_closure(move |_: &JsValue, handler_args: &[JsValue], handler_ctx: &mut Context| -> JsResult<JsValue> {
            let result = if handler_args.len() == 1 {
                handler_args[0].clone()
            } else {
                JsValue::from(JsArray::from_iter(handler_args.iter().cloned(), handler_ctx))
            };
...
fn build_class(ctx: &mut Context) -> JsValue {
    ...
    let _ = ctor_obj.set(js_string!("prototype"), JsValue::from(proto.clone()), false, ctx);
    ...
    let methods: &[(&str, NativeFunction, usize)] = &[
        ...
        ("prependListener", NativeFunction::from_fn_ptr(prepend_listener_method), 2),
        ("prependOnceListener", NativeFunction::from_fn_ptr(prepend_once_listener_method), 2),
        ("emit", NativeFunction::from_fn_ptr(emit_method), 1),
        ("removeListener", NativeFunction::from_fn_ptr(remove_listener_method), 2),
        ("off", NativeFunction::from_fn_ptr(remove_listener_method), 2),
        ("removeAllListeners", NativeFunction::from_fn_ptr(remove_all_listeners_method), 0),
        ("listenerCount", NativeFunction::from_fn_ptr(listener_count_method), 1),
        ("listeners", NativeFunction::from_fn_ptr(listeners_method), 1),
        ("eventNames", NativeFunction::from_fn_ptr(event_names_method), 0),
        ("getMaxListeners", NativeFunction::from_fn_ptr(get_max_listeners_method), 0),
        ("setMaxListeners", NativeFunction::from_fn_ptr(set_max_listeners_method), 1),
    ];
    ...
    // Static methods
    let fv: JsValue = FunctionObjectBuilder::new(ctx.realm(), NativeFunction::from_fn_ptr(static_listener_count_method))
        .name("listenerCount")
        .length(2)
        .build()
        .into();
    ...
    let fv: JsValue = FunctionObjectBuilder::new(ctx.realm(), NativeFunction::from_fn_ptr(static_once_method))
        .name("once")
        .length(2)
        .build()
        .into();
