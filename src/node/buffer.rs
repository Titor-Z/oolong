...
use boa_engine::{
    Context, Finalize, JsData, JsError, JsObject, JsResult, JsString, JsValue, Module, Trace,
    boa_class, js_string, module::SyntheticModuleInitializer,
};
...
impl Buffer {
    ...
    pub fn constructor(value: Option<JsValue>, encoding: Option<JsString>) -> JsResult<Self> {
        ...
        if let Some(n) = value.as_number() {
            let size = n as usize;
            return Ok(Self {
                data: GcRefCell::new(vec![0u8; size]),
            });
        }
        ...
        if let Some(obj) = value.as_object() {
                ...
                .and_then(|v| v.to_length(&mut ctx))
            {
                ...
                for i in 0..len {
                    if let Ok(v) = obj
                        .get(i, &mut ctx)
                        .map(|v| v.to_number(&mut ctx).unwrap_or(0.0) as u8)
                    {
                        data.push(v);
                    }
                }
    ...
    pub fn slice(&self, start: Option<usize>, end: Option<usize>) -> Self {
        ...
        Self {
            data: GcRefCell::new(data[start..end].to_vec()),
        }
    }
    ...
    pub fn isBuffer(value: JsValue) -> bool {
        value.is_object() && value.as_object().is_some_and(|o| o.is::<Buffer>())
    }
    ...
    let module = Module::synthetic(
        ...
        SyntheticModuleInitializer::from_copy_closure(
            |m: &boa_engine::module::SyntheticModule, ctx: &mut Context| {
                ...
                let _ = ns.set(
                    js_string!("kMaxLength"),
                    JsValue::from(0x7FFFFFFF as f64),
                    false,
                    ctx,
                );
