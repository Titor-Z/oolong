...
use boa_engine::{
  ...
  Context, IntoJsFunctionCopied, JsNativeError, JsObject, JsResult, JsString, JsValue,
  ...
  let module = Module::synthetic(
    ...
    unsafe {
      SyntheticModuleInitializer::from_closure(
        ...
        let _ = versions_obj.set(js_string!("oolong"), JsValue::from(js_string!("0.1.0-dev.2")), false, ctx);
        ...
        let chdir_fn = make_fn(
          (|dir: JsString, _ctx: &mut Context| -> JsResult<JsValue> {
            match std::env::set_current_dir(dir.to_std_string_escaped()) {
              ...
              Err(e) => Err(JsNativeError::typ().with_message(format!("chdir: {e}")).into()),
            }
          })
          .into_js_function_copied(ctx),
          "chdir", 1, ctx,
            ...
            match new_title {
              Some(t) => {
                ...
                { std::process::Command::new("sh")
                  .arg("-c")
                  .arg(format!("printf '\\e]0;{}\\a'", t.to_std_string_escaped().replace('\'', "'\\''")))
                  .status().ok(); }
                Ok(t.into())
              }
        ...
        let stdout_write_fn = make_fn(
          (|text: JsString, _ctx: &mut Context| -> JsResult<JsValue> {
            ...
            match out.write_all(s.as_bytes()) {
              ...
              Err(e) => Err(JsNativeError::typ().with_message(format!("stdout.write: {e}")).into()),
            }
          })
          .into_js_function_copied(ctx),
          "write", 1, ctx,
        ...
        let stderr_write_fn = make_fn(
          (|text: JsString, _ctx: &mut Context| -> JsResult<JsValue> {
            ...
            match out.write_all(s.as_bytes()) {
              Ok(()) => { out.flush().ok(); Ok(JsValue::undefined()) }
              Err(e) => Err(JsNativeError::typ().with_message(format!("stderr.write: {e}")).into()),
            }
          })
          .into_js_function_copied(ctx),
          "write", 1, ctx,
