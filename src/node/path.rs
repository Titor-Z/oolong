...
use boa_engine::{
    ...
    Module, interop::JsRest, js_string,
};
...
fn make_path_obj(win: bool, ctx: &mut Context) -> JsResult<JsObject> {
    ...
    let f = (move |rest: JsRest<'_>, _ctx: &mut Context| -> JsResult<JsValue> {
        ...
        Ok(JsValue::from(js_string!(
            normalize_internal(&parts.join(sep), win).as_str()
        )))
    })
    ...
    let f = (move |path: JsValue, _ctx: &mut Context| -> JsResult<JsValue> {
        let p = str_arg(&path)?;
        Ok(JsValue::from(js_string!(
            normalize_internal(&p, win).as_str()
        )))
    })
    .into_js_function_copied(ctx);
    let _ = obj.set(
    ...
    let f = (move |from: JsValue, to: JsValue, _ctx: &mut Context| -> JsResult<JsValue> {
        ...
        let fparts: Vec<&str> = fnorm
            .split(|ch| is_sep(ch as u8, win))
            .filter(|s| !s.is_empty())
            .collect();
        let tparts: Vec<&str> = tnorm
            .split(|ch| is_sep(ch as u8, win))
            .filter(|s| !s.is_empty())
            .collect();
        let mut i = 0;
        while i < fparts.len() && i < tparts.len() && fparts[i] == tparts[i] {
            i += 1;
        }
    ...
    let f = (move |rest: JsRest<'_>, _ctx: &mut Context| -> JsResult<JsValue> {
        ...
        for (i, p) in parts.iter().enumerate() {
            if !p.is_empty() && is_sep(p.as_bytes()[0], win) {
                abs = true;
                start = i;
                break;
            }
        ...
        Ok(JsValue::from(js_string!(
            normalize_internal(&joined, win).as_str()
        )))
    })
    .into_js_function_copied(ctx);
    let _ = obj.set(
    ...
    let f = (move |path: JsValue, ctx: &mut Context| -> JsResult<JsValue> {
        ...
        if n.is_empty() || n == "." {
            ...
            let _ = result.set(
                js_string!("base"),
                JsValue::from(js_string!(base.as_str())),
                false,
                ctx,
            );
            let _ = result.set(
                js_string!("ext"),
                JsValue::from(js_string!(ext.as_str())),
                false,
                ctx,
            );
            let _ = result.set(
    ...
    let f = (move |obj_val: JsValue, ctx: &mut Context| -> JsResult<JsValue> {
        let obj = obj_val.as_object().ok_or_else(|| {
            JsError::from(
                JsNativeError::typ().with_message("path.format: argument must be an object"),
            )
        })?;
        let dir = obj
            .get(js_string!("dir"), ctx)
            .ok()
            .and_then(|v| v.as_string().map(|s| s.to_std_string_escaped()))
            .unwrap_or_default();
        let base = if let Some(base_v) = obj
            .get(js_string!("base"), ctx)
            .ok()
            .and_then(|v| v.as_string().map(|s| s.to_std_string_escaped()))
        {
            base_v
    ...
    let module = Module::synthetic(
        ...
        SyntheticModuleInitializer::from_copy_closure(
            |m: &boa_engine::module::SyntheticModule, ctx: &mut Context| {
                ...
                let sep_v = path_obj
                    .get(js_string!("sep"), ctx)
                    .map_err(|_| JsError::from(JsNativeError::typ().with_message("no sep")))?;
                let delim_v = JsValue::from(js_string!(if cfg!(windows) { ";" } else { ":" }));
                let join_v = path_obj
                    .get(js_string!("join"), ctx)
                    .map_err(|_| JsError::from(JsNativeError::typ().with_message("no join")))?;
                let dirname_v = path_obj
                    .get(js_string!("dirname"), ctx)
                    .map_err(|_| JsError::from(JsNativeError::typ().with_message("no dirname")))?;
                let basename_v = path_obj
                    .get(js_string!("basename"), ctx)
                    .map_err(|_| JsError::from(JsNativeError::typ().with_message("no basename")))?;
                let extname_v = path_obj
                    .get(js_string!("extname"), ctx)
                    .map_err(|_| JsError::from(JsNativeError::typ().with_message("no extname")))?;
                let is_abs_v = path_obj.get(js_string!("isAbsolute"), ctx).map_err(|_| {
                    JsError::from(JsNativeError::typ().with_message("no isAbsolute"))
                })?;
                let normalize_v = path_obj.get(js_string!("normalize"), ctx).map_err(|_| {
                    JsError::from(JsNativeError::typ().with_message("no normalize"))
                })?;
                let relative_v = path_obj
                    .get(js_string!("relative"), ctx)
                    .map_err(|_| JsError::from(JsNativeError::typ().with_message("no relative")))?;
                let resolve_v = path_obj
                    .get(js_string!("resolve"), ctx)
                    .map_err(|_| JsError::from(JsNativeError::typ().with_message("no resolve")))?;
                let parse_v = path_obj
                    .get(js_string!("parse"), ctx)
                    .map_err(|_| JsError::from(JsNativeError::typ().with_message("no parse")))?;
                let format_v = path_obj
                    .get(js_string!("format"), ctx)
                    .map_err(|_| JsError::from(JsNativeError::typ().with_message("no format")))?;
                let to_ns_v = path_obj
                    .get(js_string!("toNamespacedPath"), ctx)
                    .map_err(|_| {
                        JsError::from(JsNativeError::typ().with_message("no toNamespacedPath"))
                    })?;
