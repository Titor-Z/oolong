...
fn atob_impl(_: &JsValue, args: &[JsValue], _ctx: &mut Context) -> JsResult<JsValue> {
    ...
    let bytes = engine.decode(s).map_err(|e| {
        JsNativeError::typ().with_message(format!("atob: {e}"))
    })?;
    let result =
        String::from_utf8(bytes).map_err(|_| JsNativeError::typ().with_message("atob: invalid UTF-8 after decode"))?;
...
pub fn register_globals(context: &mut Context) -> JsResult<()> {
    let atob_fn = FunctionObjectBuilder::new(
        context.realm(),
        NativeFunction::from_fn_ptr(atob_impl),
    )
    .name(js_string!("atob"))
    .length(1)
    .build();
    ...
    let btoa_fn = FunctionObjectBuilder::new(
        context.realm(),
        NativeFunction::from_fn_ptr(btoa_impl),
    )
    .name(js_string!("btoa"))
    .length(1)
    .build();
