...
impl Blob {
    ...
    pub fn constructor(parts: Option<JsValue>, options: Option<JsObject>) -> JsResult<Self> {
        ...
        if let Some(parts) = parts {
            let parts = parts
                .as_object()
                .ok_or_else(|| js_error!(TypeError: "Blob parts must be an array or iterable"))?;
            let length = parts
                .get(js_string!("length"), &mut Context::default())
                .and_then(|v| v.to_length(&mut Context::default()))
                .unwrap_or(0);
            for i in 0..length {
                let item = parts
                    .get(i, &mut Context::default())
    ...
    pub fn slice(
        ...
        let relative_start = start.map_or(0, |s| if s < 0 { (len + s).max(0) } else { s.min(len) });
        let relative_end = end.map_or(len, |e| if e < 0 { (len + e).max(0) } else { e.min(len) });
...
impl JsFile {
    ...
    pub fn constructor(
        ...
        let parts_obj = parts
            .as_object()
            .ok_or_else(|| js_error!(TypeError: "File parts must be an array or iterable"))?;
        let length = parts_obj
            .get(js_string!("length"), &mut Context::default())
            .and_then(|v| v.to_length(&mut Context::default()))
            .unwrap_or(0);
        for i in 0..length {
