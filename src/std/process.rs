  ...
  let module = Module::synthetic(
    ...
    SyntheticModuleInitializer::from_copy_closure(
      |m: &boa_engine::module::SyntheticModule, ctx: &mut Context| {
        ...
        let cwd_fn = make_fn(
          (|_: &mut Context| -> JsResult<JsValue> {
            match std::env::current_dir() {
              ...
              Err(e) => Err(JsNativeError::typ().with_message(format!("cwd failed: {e}")).into()),
            }
          })
        ...
        // platform
        m.set_export(&js_string!("platform"), JsValue::from(js_string!(platform_str())))?;
        ...
        for (k, v) in std::env::vars() {
          let _ = env_obj.set(JsString::from(k), JsValue::from(JsString::from(v)), false, ctx);
        }
        ...
        for (i, a) in std::env::args().enumerate() {
          let _ = argv_arr.set(i as u32, JsValue::from(JsString::from(a)), false, ctx);
        }
        ...
        let _ = versions_obj.set(js_string!("oolong"), JsValue::from(js_string!("0.1.0-dev.1")), false, ctx);
        ...
        let exec_path_fn = make_fn(
          (|_: &mut Context| -> JsResult<JsValue> {
            match std::env::current_exe() {
              Ok(p) => Ok(JsValue::from(JsString::from(p.to_string_lossy().to_string()))),
              Err(e) => Err(JsNativeError::typ().with_message(format!("execPath: {e}")).into()),
            }
          })
          .into_js_function_copied(ctx),
          "execPath",
        ...
        let stdout_write_fn = make_fn(
          (|text: JsString, _ctx: &mut Context| -> JsResult<JsValue> {
            ...
            match out.write_all(s.as_bytes()) {
              Ok(()) => { out.flush().ok(); Ok(JsValue::undefined()) }
              Err(e) => Err(JsError::from(
        ...
        let stdin_read_fn = make_fn(
          (|ctx: &mut Context| -> JsResult<JsValue> {
            ...
            match std::io::stdin().lock().read_to_string(&mut input) {
              ...
              Ok(_) => Ok(JsPromise::resolve(JsValue::from(JsString::from(input)), ctx).into()),
              Err(e) => {
        ...
        let stdin_read_bytes_fn = make_fn(
          (|ctx: &mut Context| -> JsResult<JsValue> {
            ...
            match std::io::stdin().lock().read_to_end(&mut buf) {
              Ok(0) => Ok(JsPromise::resolve(JsValue::null(), ctx).into()),
              Ok(_) => {
                match JsArrayBuffer::new(buf.len(), ctx) {
                  Ok(ab) => {
                  ...
                  Err(e) => {
                    ...
                    let _ = resolvers.reject.call(
                      &JsValue::undefined(),
                      &[JsValue::from(JsString::from(format!("stdin.readAsBytes: buffer alloc: {e}")))],
                      ctx,
                    );
                    Ok(promise.into())
                  }
              ...
              Err(e) => {
                ...
                let _ = resolvers.reject.call(
                  &JsValue::undefined(),
                  &[JsValue::from(JsString::from(format!("stdin.readAsBytes: {e}")))],
                  ctx,
                );
                Ok(promise.into())
              }
            ...
            match new_title {
              Some(t) => {
                ...
                std::process::Command::new("sh")
                  .arg("-c")
                  .arg(format!("echo -n ''; printf '\\e]0;{}\\a'", s.replace('\'', "'\\''")))
                  .status()
                  .ok();
                Ok(JsValue::from(t))
              }
        ...
        for (i, a) in std::env::args().enumerate() {
          if i > 0 {
            let _ = exec_argv_arr.push(JsValue::from(JsString::from(a)), ctx);
          }
        }
        ...
        let _ = pobj.set(js_string!("pid"), JsValue::from(std::process::id()), false, ctx);
        let _ = pobj.set(js_string!("ppid"), JsValue::from(ppid_val), false, ctx);
        let _ = pobj.set(js_string!("platform"), JsValue::from(js_string!(platform_str())), false, ctx);
        let _ = pobj.set(js_string!("arch"), JsValue::from(js_string!(arch_str())), false, ctx);
        let _ = pobj.set(js_string!("title"), title_fn, false, ctx);
        let _ = pobj.set(js_string!("version"), JsValue::from(js_string!("0.1.0")), false, ctx);
