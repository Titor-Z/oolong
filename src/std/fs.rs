...
use boa_engine::{
    Context, Finalize, IntoJsFunctionCopied, JsData, JsError, JsNativeError, JsObject, JsResult,
    JsString, JsValue, Module, NativeFunction, Trace, js_string, object::FunctionObjectBuilder,
};
...
fn metadata_to_obj(md: &std::fs::Metadata, ctx: &mut Context) -> JsResult<JsValue> {
    ...
    let _ = obj.set(
        js_string!("mode"),
        JsValue::from(md.permissions().mode() as f64),
        false,
        ctx,
    );
    #[cfg(not(unix))]
...
fn is_recursive(opts: Option<JsValue>, ctx: &mut Context) -> bool {
    match opts {
        Some(v) if v.is_object() => {
            ...
            obj.get(js_string!("recursive"), ctx)
                .map(|v| v.to_boolean())
                .unwrap_or(false)
        }
        _ => false,
            ...
            |m: &boa_engine::module::SyntheticModule, ctx: &mut Context| {
                ...
                let read_sync = to_js_fn(
                    (|path: JsString, ctx: &mut Context| -> JsResult<JsValue> {
                        ...
                        match std::fs::read(&p) {
                            Ok(bytes) => {
                                let buf =
                                    JsArrayBuffer::new(bytes.len(), ctx).map_err(|e| {
                                        JsError::from(JsNativeError::typ().with_message(format!(
                                            "readFileSync: buffer alloc: {e}"
                                        )))
                                    })?;
                            ...
                            Err(e) => Err(JsError::from(
                                JsNativeError::typ().with_message(format!("readFileSync: {e}")),
                            )),
                        }
                    })
                    .into_js_function_copied(ctx),
                    ...
                    (|path: JsString, ctx: &mut Context| -> JsResult<JsValue> {
                        ...
                        match std::fs::read(&p) {
                            Ok(bytes) => match JsArrayBuffer::new(bytes.len(), ctx) {
                                ...
                                Err(e) => Ok(rejected_promise(
                                    &format!("readFile: buffer alloc: {e}"),
                                    ctx,
                                )),
                            },
                ...
                let write_file = to_js_fn(
                    (|path: JsString, data: JsValue, ctx: &mut Context| -> JsResult<JsValue> {
                        ...
                        match data_bytes {
                            ...
                            None => Ok(rejected_promise(
                                "writeFile: data must be a string, ArrayBuffer, or TypedArray",
                                ctx,
                            )),
                        }
                    })
                ...
                let mkdir_fn = to_js_fn(
                    (|path: JsString,
                     ...
                     -> JsResult<JsValue> {
                        let ps = path.to_std_string_escaped();
                        let r = if is_recursive(opts, ctx) {
                            std::fs::create_dir_all(&ps)
                        } else {
                            std::fs::create_dir(&ps)
                        };
                        match r {
                            Ok(()) => Ok(resolved_promise(JsValue::undefined(), ctx)),
                ...
                let append_fn = to_js_fn(
                    (|path: JsString, data: JsValue, ctx: &mut Context| -> JsResult<JsValue> {
                        ...
                        match data.to_string(ctx) {
                                ...
                                .open(&p)
                            {
                                Ok(mut f) => {
                                    ...
                                    match f.write_all(s.to_std_string_escaped().as_bytes()) {
                                        Ok(()) => Ok(resolved_promise(JsValue::undefined(), ctx)),
                                        Err(e) => {
                                            Ok(rejected_promise(&format!("appendFile: {e}"), ctx))
                                        }
                                    }
                                }
                            ...
                            Err(_) => {
                                Ok(rejected_promise("appendFile: data must be a string", ctx))
                            }
                        }
                    })
                    .into_js_function_copied(ctx),
                ...
                let copy_fn = to_js_fn(
                    (|src: JsString, dst: JsString, ctx: &mut Context| -> JsResult<JsValue> {
                        match std::fs::copy(
                            src.to_std_string_escaped(),
                            dst.to_std_string_escaped(),
                        ) {
                ...
                let rename_fn = to_js_fn(
                    (|old: JsString, new: JsString, ctx: &mut Context| -> JsResult<JsValue> {
                        match std::fs::rename(
                            old.to_std_string_escaped(),
                            new.to_std_string_escaped(),
                        ) {
                            Ok(()) => Ok(resolved_promise(JsValue::undefined(), ctx)),
                ...
                let realpath_fn = to_js_fn(
                    (|path: JsString, ctx: &mut Context| -> JsResult<JsValue> {
                        match std::fs::canonicalize(path.to_std_string_escaped()) {
                            Ok(p) => Ok(resolved_promise(
                                JsValue::from(JsString::from(p.to_string_lossy().to_string())),
                                ctx,
                            )),
                    ...
                    (|path: JsString,
                     ...
                     -> JsResult<JsValue> {
                        ...
                        match r {
                            ...
                            Err(e) => Err(JsError::from(
                                JsNativeError::typ().with_message(format!("mkdirSync: {e}")),
                            )),
                        }
                    })
                    .into_js_function_copied(ctx),
                    ...
                    (|path: JsString,
                     ...
                     -> JsResult<JsValue> {
                        ...
                        match r {
                            Ok(()) => Ok(JsValue::undefined()),
                            Err(e) => Err(JsError::from(
                                JsNativeError::typ().with_message(format!("removeSync: {e}")),
                            )),
                        }
                    })
                    .into_js_function_copied(ctx),
                ...
                let readdir_sync = to_js_fn(
                    (|path: JsString, ctx: &mut Context| -> JsResult<JsValue> {
                        ...
                        match std::fs::read_dir(&p) {
                            ...
                            Err(e) => Err(JsError::from(
                                JsNativeError::typ().with_message(format!("readdirSync: {e}")),
                            )),
                        }
                    })
                    .into_js_function_copied(ctx),
                ...
                let stat_sync = to_js_fn(
                    (|path: JsString, ctx: &mut Context| -> JsResult<JsValue> {
                        ...
                        match std::fs::metadata(&p) {
                            ...
                            Err(e) => Err(JsError::from(
                                JsNativeError::typ().with_message(format!("statSync: {e}")),
                            )),
                        }
                    })
                    .into_js_function_copied(ctx),
                ...
                let lstat_sync = to_js_fn(
                    (|path: JsString, ctx: &mut Context| -> JsResult<JsValue> {
                        ...
                        match std::fs::symlink_metadata(&p) {
                            Ok(md) => metadata_to_obj(&md, ctx),
                            Err(e) => Err(JsError::from(
                                JsNativeError::typ().with_message(format!("lstatSync: {e}")),
                            )),
                        }
                    })
                    .into_js_function_copied(ctx),
                    ...
                    (|path: JsString, data: JsValue, _ctx: &mut Context| -> JsResult<JsValue> {
                        match data.to_string(_ctx) {
                            Ok(s) => match std::fs::OpenOptions::new()
                                .append(true)
                                .create(true)
                                .open(path.to_std_string_escaped())
                            {
                                Ok(mut f) => {
                                    use std::io::Write;
                                    f.write_all(s.to_std_string_escaped().as_bytes()).map_err(
                                        |e| {
                                            JsError::from(
                                                JsNativeError::typ()
                                                    .with_message(format!("appendFileSync: {e}")),
                                            )
                                        },
                                    )?;
                            ...
                            Err(_) => Err(JsError::from(
                                JsNativeError::typ()
                                    .with_message("appendFileSync: data must be a string"),
                            )),
                        }
                    })
                    .into_js_function_copied(ctx),
                ...
                let copy_sync = to_js_fn(
                    (|src: JsString, dst: JsString, _ctx: &mut Context| -> JsResult<JsValue> {
                        std::fs::copy(src.to_std_string_escaped(), dst.to_std_string_escaped())
                            .map_err(|e| {
                                JsError::from(
                                    JsNativeError::typ().with_message(format!("copyFileSync: {e}")),
                                )
                            })?;
                        Ok(JsValue::undefined())
                    })
                ...
                let rename_sync = to_js_fn(
                    (|old: JsString, new: JsString, _ctx: &mut Context| -> JsResult<JsValue> {
                        std::fs::rename(old.to_std_string_escaped(), new.to_std_string_escaped())
                            .map_err(|e| {
                            JsError::from(
                                JsNativeError::typ().with_message(format!("renameSync: {e}")),
                            )
                        })?;
                        Ok(JsValue::undefined())
                    })
                ...
                let realpath_sync = to_js_fn(
                    (|path: JsString, _ctx: &mut Context| -> JsResult<JsValue> {
                        match std::fs::canonicalize(path.to_std_string_escaped()) {
                            ...
                            Err(e) => Err(JsError::from(
                                JsNativeError::typ().with_message(format!("realpathSync: {e}")),
                            )),
                        }
                    })
                    .into_js_function_copied(ctx),
                ...
                let symlink_sync = to_js_fn(
                    (|target: JsString, path: JsString, _ctx: &mut Context| -> JsResult<JsValue> {
                        #[cfg(unix)]
                        {
                            ...
                            .map_err(|e| {
                                JsError::from(
                                    JsNativeError::typ().with_message(format!("symlinkSync: {e}")),
                                )
                            })?;
                            Ok(JsValue::undefined())
                        }
                        #[cfg(not(unix))]
                        {
                            ...
                            Err(JsError::from(
                                JsNativeError::typ()
                                    .with_message("symlinkSync: only supported on Unix"),
                            ))
                        }
                    })
                    .into_js_function_copied(ctx),
                ...
                let link_fn = to_js_fn(
                    (|existing: JsString, new: JsString, ctx: &mut Context| -> JsResult<JsValue> {
                        match std::fs::hard_link(
                            existing.to_std_string_escaped(),
                            new.to_std_string_escaped(),
                        ) {
                            Ok(()) => Ok(resolved_promise(JsValue::undefined(), ctx)),
                ...
                let access_fn = to_js_fn(
                    (|path: JsString,
                     ...
                     -> JsResult<JsValue> {
                        ...
                        if m == 0 {
                            return Ok(resolved_promise(JsValue::undefined(), ctx));
                        }
                        #[cfg(unix)]
                        {
                            use std::os::unix::fs::PermissionsExt;
                                ...
                                Ok(md) => {
                                    ...
                                    if (m & 4) != 0 && (perm & 0o444) == 0 {
                                        return Ok(rejected_promise(
                                            &format!("access: EACCES, not readable: {ps}"),
                                            ctx,
                                        ));
                                    }
                                    if (m & 2) != 0 && (perm & 0o222) == 0 {
                                        return Ok(rejected_promise(
                                            &format!("access: EACCES, not writable: {ps}"),
                                            ctx,
                                        ));
                                    }
                                    if (m & 1) != 0 && (perm & 0o111) == 0 {
                                        return Ok(rejected_promise(
                                            &format!("access: EACCES, not executable: {ps}"),
                                            ctx,
                                        ));
                                    }
                    ...
                    (|path: JsString,
                     ...
                     -> JsResult<JsValue> {
                        ...
                        if poll_needed {
                                ...
                                for entry in entries.iter_mut() {
                                    ...
                                    if current != entry.last_mtime && current != 0 {
                                        ...
                                        let iter_result = {
                                            ...
                                            let _ =
                                                ir.set(js_string!("value"), event_obj, false, ctx);
                                            let _ = ir.set(
                                                js_string!("done"),
                                                JsValue::from(false),
                                                false,
                            ...
                            fn schedule_poll(
                                state: Gc<GcRefCell<FsWatchState>>,
                                ctx: &mut Context,
                            ) {
                                ctx.enqueue_job(
                                    TimeoutJob::new(
                        ...
                        // 创建 watcher 对象
                        let _async_iter_proto = ctx
                            .intrinsics()
                            .objects()
                            .iterator_prototypes()
                            .async_iterator();
                        let watcher_obj = JsObject::with_object_proto(ctx.intrinsics());
                        let _ = watcher_obj.set(
                            js_string!("__watch_id"),
                        ...
                        let next_fn = {
                            ...
                            unsafe {
                                NativeFunction::from_closure_with_captures(
                                    move |this: &JsValue,
                                          _args: &[JsValue],
                                          captures: &Gc<GcRefCell<FsWatchState>>,
                                          ctx: &mut Context|
                                          -> JsResult<JsValue> {
                                        let id = this
                                            .as_object()
                                        ...
                                        match entry {
                                            ...
                                            Some(entry) if !entry.pending.is_empty() => {
                                                let resolve =
                                                    entry.pending.front().unwrap().clone();
                                                let ir =
                                                    JsObject::with_object_proto(ctx.intrinsics());
                                                let _ = ir.set(
                                                    js_string!("value"),
                                                ...
                                                let (promise, _resolvers) =
                                                    JsPromise::new_pending(ctx);
                                                Ok(promise.into())
                                            }
                                            Some(_) => {
                                                // 没有排队事件，创建新 promise 等待轮询
                                                let (promise, resolvers) =
                                                    JsPromise::new_pending(ctx);
                                                // 从 borrow 中释放才能插入
                                                drop(entries);
                                                let mut entries_mut = s.entries.borrow_mut();
                                                if let Some(e) =
                                                    entries_mut.iter_mut().find(|e| e.id == id)
                                                {
                                            ...
                                            None => {
                                                Ok(JsPromise::resolve(JsValue::undefined(), ctx)
                                                    .into())
                                            }
                                        }
                                    },
                        ...
                        let async_iter_val =
                            to_js_fn(async_iter_fn, "[Symbol.asyncIterator]", 0, ctx);
                        let _ =
                            watcher_obj.set(JsSymbol::async_iterator(), async_iter_val, false, ctx);
